...
export default class practoService {
  ...
  public async getAllInvoices(IFilterDTO: IFilterDTO, currentUser: IUser): Promise<{ data: any }> {
    try {
      ...
      var patientLoans = await this.patientLoanModel
        ...
        .limit(pageSize);
      var data = { patientLoans, numberOfPages };
      return { data };
    } catch (e) {
      this.logger.error(e);
      throw e;
  ...
  public async postPatientTermLoan(
    ...
    try {
      // const product = await this.patientLoanModel.findOne({ 'productCode': productCode });

      // if (product && product.productCode) {
      //     return {
      //         data: {
      //             success: false,
      //             message: 'patientLoan code already exists',
      //         }, responseCode: 400
      //     }
      // }
